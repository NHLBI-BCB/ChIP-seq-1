...
#Takes two peak files and determines which peaks are overlapping

#Input1:Bed file for peaks 1 - chr loc
#Input2:Bed file for peaks 2 - chr loc
#window:window for distance between peaks


# If we aren't given three command line arguments, die with an error that explains the correct arguments
if(scalar(@ARGV) != 3) { die "Usage: neighboring_peaks.pl peaks1 peaks2 window"; }
my ($peaks1, $peaks2, $window) = @ARGV;

# Read first peak file into the regions hash, which maps each chromosome to a list of hashes, each of the form:
# {loc=>, start=>start of region around loc, stop=>end of region around loc}
my $peakregions = readpeaks($peaks1, $window \&bp2WinIdentity: \&bp2WinWindowedInput);

#print "regions=",obj2Str($regions, ""),"\n";
#print "------------------------\n";

# Load the second peak file, align to first peak file, updating the %$regions hash
my $peakregions2 = loadReadsAndAlignToRegions($peaks2, $regions, $window<=1? \&bp2WinIdentity: \&bp2WinWindowedInput);
...
sub readpeaks
{
	my ($peaksfile, $windowsize, $bp2Win) = @_;

	open(my $peaks, "<$peaksfile")  || die "ERROR opening file \"$peaksfile\"! $!"; 
	...
	while(my $line = <$peaks>) {
		...
		my $region;
		my ($chr, $loc);
		if(scalar(@data)>=2 && $windowsize>0) { 
		  ($chr, $loc) = @data;
		  $region = {locBP=>$loc, loc=>$bp2Win->($loc), start=>$bp2Win->($loc)-$windowsize, stop=>$bp2Win->($loc)+$windowsize};
		} elsif(scalar(@data)>=3 && $windowsize<=0) { 
		  ($chr, $start, $stop) = @data;
		  $region = {locBP=>int(($stop-$start)/2), loc=>$bp2Win->(int(($stop-$start)/2)), start=>$bp2Win->($start), stop=>$bp2Win->($stop)};
		} else
		{ die "ERROR in \"$peaksfile\":$locLNum expected 2 fields!"; }
